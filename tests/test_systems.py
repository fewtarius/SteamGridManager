...
from systems import (
    SystemDef,
    EmulatorConfigWrapper,
    get_system,
    list_supported_systems,
    find_vita3k_data_dir,
    _find_vita3k_binary,
)
from emulators import EmulatorPlugin
...
class TestSystemDefinitions:
    ...
    def test_psvita_has_emulator(self):
        ...
        assert sys_def.launch_mode == 'title_id'
        assert '-F -r "{title_id}"' in sys_def.emulator.launch_args

    def test_psvita_has_platform_ids(self):
        """PS Vita has TheGamesDB ID."""
        sys_def = get_system('psvita')
        # Note: screenscraper_id may be 0 if not configured
        assert sys_def.thegamesdb_id == '19'
    ...
    def test_psvita_legacy_tags(self):
        ...
        assert 'PlayStation Vita' in sys_def.legacy_tags
    ...
    def test_all_systems_have_required_fields(self):
        """Every system has id, fullname, manufacturer, emulator."""
        for name in list_supported_systems():
            sys_def = get_system(name)
            assert sys_def.id == name
...
class TestVita3KEmulator:
    ...
    def test_steam_exe_with_title_id(self):
        ...
        sys_def = get_system('psvita')
        with patch.object(sys_def.emulator, 'find_executable', return_value='/usr/bin/Vita3K'):
...
class TestEmulatorConfigWrapper:
    """Test EmulatorConfigWrapper for all launch modes."""

    def test_rom_launch_mode(self):
        """Standard ROM launch mode works."""
        plugin = EmulatorPlugin('test-ra', {
            'type': 'retroarch',
            'core': 'snes9x',
            'launch_args': '-L /core.so "{rom}"',
            'launch_mode': 'rom',
        })
        emu = EmulatorConfigWrapper(plugin)
        with patch.object(plugin, 'find_executable', return_value='/usr/bin/retroarch'):
    ...
    def test_title_id_launch_mode(self):
        ...
        plugin = EmulatorPlugin('test-vita3k', {
            'type': 'standalone',
            'executable': '/usr/bin/Vita3K',
            'launch_args': '-F -r "{title_id}"',
            'launch_mode': 'title_id',
        })
        emu = EmulatorConfigWrapper(plugin)
        with patch.object(plugin, 'find_executable', return_value='/usr/bin/Vita3K'):
    ...
    def test_flatpak_launch_mode(self):
        ...
        plugin = EmulatorPlugin('test-ra-flatpak', {
            'type': 'retroarch',
            'core': 'snes9x',
